...
// Assets
import { MdCancel, MdCheckCircle, MdLogin, MdLogout, MdOutlineCached, MdOutlineCalendarToday, MdOutlineError} from 'react-icons/md';
...
export default function TablePointage(props: { tableData: any,updateDefaultData:() => void}) {
	...
	const handleClickSortir = (id:number) =>{
		...
		if(authData) {
			axios.put(`http://localhost:3001/api/pointage/${id}`, {
				employee_id :id,
		...
		columnHelper.accessor('date_heure_sortie', {
			...
			cell: (info) => {
				...
				return (
					...
					(info.getValue() == null ? (
						<Flex align='center'>
							<Button bg={"red.500"} onClick={()=> handleClickSortir(info.row.original.employee_id)} _hover={bgHover1} fontSize='xs' fontWeight='800'  color={textColorSecondary} borderRadius='7px'>
								<Icon as={MdLogout} color={textColorSecondary} fontWeight='800' me='4px' />
								Sortie
							</Button>
						</Flex>
					) : (
	...
	const handleDelete = async (itemId:any) => {
		...
		if (result.isConfirmed) {
			...
			try {
				axios.delete(`http://localhost:3001/api/pointage/${itemId}`).then(()=>{
					Toast.fire({
						icon: 'success',
						title: 'Success'
					})
	...
	return (
		<Card flexDirection='column' w='100%' px='0px'>
			...
			<Box w='100%' px='0px' overflowX={{ sm: 'scroll'}}>
				...
				{idEdit !== null &&(
					<RemarquePointage isOpen={isEditModalOpen} onClose={closeEditModal} updateDefaultData={props.updateDefaultData} id={idEdit}/>
				)}
			</Box>
		</Card>
	);
