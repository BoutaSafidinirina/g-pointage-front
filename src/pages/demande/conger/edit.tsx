...
export const EditConger: FunctionComponent<AddModalProps> = ({ isOpen, onClose,updateDefaultData ,id}) => {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            const formatDateForInput = (backendDate:any) => {
                const dateObject = new Date(backendDate);
                const formattedDate = dateObject.toISOString().split('T')[0];
                return formattedDate;
            };

            const response : any = await axios.get(`http://localhost:3001/api/conge/info/${id}`)
            setForm({
                date_debut:{ value : formatDateForInput(response?.data.date_debut)},
                date_fin:{ value : formatDateForInput(response?.data.date_fin)},
                ...
                type_conge_id:{value : response?.data.type_conge_id}
            })
            console.log(response?.data.date_fin)
            console.log(form)
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            await axios.put(`http://localhost:3001/api/conge/${id}`, formData, {
                headers: {
