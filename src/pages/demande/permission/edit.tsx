...
export const EditPermission: FunctionComponent<AddModalProps> = ({ isOpen, onClose,updateDefaultData , id}) => {
    ...
    const [form, setForm] = useState<Form>({
        heure_debut: { value: null },
        heure_fin: { value: null },
        motif: { value: ""},
        destinataire: { value: "" },
        employee_id: { value: 0 },
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            await axios.put(`http://localhost:3001/api/permission/${id}`, formData, {
                headers: {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            const response :any = await axios.get(`http://localhost:3001/api/permission/info/${id}`)
            const formatToDateTimeLocal = (dateString:any) => {
            ...
            setForm({
                heure_debut:{value: formatToDateTimeLocal(response?.data.heure_debut)},
                heure_fin: {value: formatToDateTimeLocal(response?.data.heure_fin)},
                motif: {value: response?.data.motif},
                piece_justificative:{value: response?.data.piece_justificative},
                destinataire:{value: response?.data.destinataire},
                employee_id: {value: response?.data.employee_id}
            })
            setSelectedFileName(response?.data.piece_justificative);
        }
        fetchData();
    }, []);
