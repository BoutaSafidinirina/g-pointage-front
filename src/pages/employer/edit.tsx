...
export const EditEmployer: FunctionComponent<AddModalProps> = ({
    isOpen,
    onClose,
    updateDefaultData,
    id
    ...
    useEffect(() => {
        async function fetchData() {
            const response :any = await axios.get(`http://localhost:3001/api/employee/${id}`);
            setForm({
                im: { value : response?.data.im},
                nom: { value : response?.data.nom},
                prenom: { value : response?.data.prenoms},
                adresse:{ value : response?.data.adresse},
                agence:{ value : response?.data.agence},
                email:{ value : response?.data.email},
                tel:{ value : response?.data.tel},
                poste:{ value : response?.data.poste_id},
                photo:{ value : response?.data.photo}
            })
            setSelectedFileName(response?.data.photo);
