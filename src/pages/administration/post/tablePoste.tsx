...
export default function ListeTypeConger(props: { tableData:any, updateDefaultData:() => void}) {
    ...
    const openAddPosteModal = () => {
        setIsAddPosteOpen(true);
    };

    const closeAddPosteModal = () => {
        setIsAddPosteOpen(false);
    };
	const openEditPosteModal = () => {
        setIsEditPosteOpen(true);
    };

    const closeEditPosteModal = () => {
		setId(null)
        setIsEditPosteOpen(false);
    };
	...
	const columns = [
		...
		columnHelper.accessor('superieur', {
			id: 'superieur',
			header: () => (
				<Text
					justifyContent='space-between'
	...
	const handleDelete = async (itemId:any) => {
		...
		if (result.isConfirmed) {
			...
			try {
				axios.delete(`http://localhost:3001/api/post/${itemId}`).then(()=>{
					Toast.fire({
						icon: 'success',
						title: 'Success'
					})
	...
	return (
		<Card flexDirection='column' w='100%' px='0px'>
			<Flex px='25px' mb="8px" justifyContent='space-between' align='center'>
				<Text color={"navy.800"}>Liste des Postes</Text>
				<Flex px='25px' mb="8px" justifyContent='space-between' align='center'>
					<Button
						bg={"#1A62ED"}
						alignItems='center'
			...
			<AddPost isOpen={isAddPosteOpen} onClose={closeAddPosteModal} updateDefaultData={props.updateDefaultData} />
			{id !== null && (
				<EditPost isOpen={isEditPosteOpen} onClose={closeEditPosteModal} updateDefaultData={props.updateDefaultData} id={id} />
      		)}
		</Card>
	);
}
