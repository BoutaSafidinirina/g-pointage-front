...
function Sidebar(props: { routes: RoutesType[]; [x: string]: any }) {
	...
	// Chakra Color Mode
	let sidebarBg = useColorModeValue('#1a62ed', '#1a62ed');
...
export function SidebarResponsive(props: { routes: RoutesType[] }) {
	let sidebarBackgroundColor = useColorModeValue('#1a62ed', '#1a62ed');
	let menuColor = useColorModeValue('navy.800', 'navy.800');
	...
	return (
		<Flex display={{ sm: 'flex', xl: 'none' }} alignItems='center'>
			<Flex w='max-content' h='max-content' onClick={onOpen} cursor='pointer'>
				<Icon
					as={IoMenuOutline}
					color={menuColor}
					my='auto'
